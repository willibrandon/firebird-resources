...
public static class FirebirdEFCoreExtensions
{
    ...
    /// <exception cref="InvalidOperationException">Thrown when mandatory <see cref="FirebirdEFCoreSettings.ConnectionString"/> is not provided.</exception>
    public static void AddFirebirdDbContext<[DynamicallyAccessedMembers(RequiredByEF)] TContext>(
        this IHostApplicationBuilder builder,
        string connectionName,
        Action<FirebirdEFCoreSettings>? configureSettings = null,
        Action<DbContextOptionsBuilder>? configureDbContextOptions = null) where TContext : DbContext
    {
        ...
        var settings = builder.GetDbContextSettings<TContext, FirebirdEFCoreSettings>(
        ...
        void ConfigureDbContext(DbContextOptionsBuilder dbContextOptionsBuilder)
        {
            ...
            dbContextOptionsBuilder.UseFirebird(settings.ConnectionString, builder =>
            {
                ...
                {
                    builder.ExecutionStrategy(c => new FirebirdRetryingExecutionStrategy(c));
                }
    ...
    public static void EnrichFirebirdDbContext<[DynamicallyAccessedMembers(RequiredByEF)] TContext>(
            this IHostApplicationBuilder builder,
            Action<FirebirdEFCoreSettings>? configureSettings = null) where TContext : DbContext
    {
        ArgumentNullException.ThrowIfNull(builder);
            ...
            if (!settings.DisableRetry || settings.CommandTimeout.HasValue)
            {
                builder.PatchServiceDescriptor<TContext>(optionsBuilder => optionsBuilder.UseFirebird(settings.ConnectionString, options =>
                {
                    ...
                    if (!settings.DisableRetry)
                    {
                        ...
                        {
                            if (executionStrategy is FirebirdRetryingExecutionStrategy)
                            {
                            ...
                            else if (executionStrategy.GetType() != typeof(FirebirdRetryingExecutionStrategy))
                            {
                                // Check FbRetryingExecutionStrategy specifically (no 'is'), any sub-class is treated as a custom strategy.

                                throw new InvalidOperationException($"{nameof(FirebirdEFCoreSettings)}.{nameof(FirebirdEFCoreSettings.DisableRetry)} needs to be set when a custom Execution Strategy is configured.");
                            }
                            else
                            {
                                //options.EnableRetryOnFailure();
                                options.ExecutionStrategy(c => new FirebirdRetryingExecutionStrategy(c));
                            }
                        }
                        else
                        {
                    ...
                    if (settings.CommandTimeout.HasValue)
                    {
                        ...
                        {
                            throw new InvalidOperationException($"Conflicting values for 'CommandTimeout' were found in {nameof(FirebirdEFCoreSettings)} and set in DbContextOptions<{typeof(TContext).Name}>.");
                        }
